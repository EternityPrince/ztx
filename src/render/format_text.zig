...
const printTreeSummary = @import("render_tree.zig").printTreeSummary;
...
pub fn printText(writer: anytype, allocator: std.mem.Allocator, result: *const model.ScanResult, config: *const cli.Config) !void {
    ...
    if (config.show_stats) {
        try printStats(writer, allocator, result, context);
        need_gap = true;
    }
    ...
    if (config.show_tree) {
        ...
        if (!config.show_stats) {
            try printTreeSummary(writer, allocator, result, context);
            if (!config.compact) try writer.writeAll("\n");
        }
